...
from past.builtins import basestring  # Kimmo H.
...
class Pinterest:
    ...
    def extract_user_data(self, html_page=''):
        ...
        if html_page:
            index = html_page.rfind('application/json'.encode('utf-8'))
            if index >= 0:
                s = html_page[index:]
                if s and s.rfind(self.username_or_email.encode('utf-8')) > -1:
                    s = s[s.find('{'.encode('utf-8')): s.find('</script>'.encode('utf-8'))]
                    s = json.loads(s)
                    try:
                        user = s['context']['user']
                        return user
    ...
    def search(self, scope, query, next_page=False):
        ...
        r = self.get(url=url)
        html = r.content[r.content.find('application/json'.encode('utf-8')):]
        html = html[html.find('{'.encode('utf-8')):html.find('</script>'.encode('utf-8'))]
        search_result = json.loads(html)
        results = []
        try:
            if len(search_result['resources']['data']['BaseSearchResource']) > 0:
                for firstKey in search_result['resources']['data']['BaseSearchResource'].keys():
                    search_resource = search_result['resources']['data']['BaseSearchResource'][firstKey]
        ...
        for result in results:
            if result['type'] == 'pin':
                ...
                pins.append({
                    ...
                    'title': result['attribution']['title'] if result['attribution'] else (result['title'] if result['title'] else result['grid_title']),
