...
class googleimagesdownload:
    ...
    # by Kimmo H.
    def similar_images_upload_image(self, image_path):
        try:
            print('similar_images_upload_image')
            #searchUrl = 'https://www.google.com/searchbyimage?site=search&sa=X&image_url=' + similar_images
            searchUrl = 'https://www.google.com/searchbyimage/upload'
            multipart = {'image_url': '', 'btnG': 'Search by image', 'encoded_image': (image_path, open(image_path, 'rb')), 'image_content': '', 'filename': '', 'hl': 'en'}
            headers = {}
            ...
            headers['Content-Type'] = 'multipart/form-data; boundary=---------------------------183761209019759'
            headers['Content-Length'] = '429872'
            headers['Connection'] = 'keep-alive'
            headers['DNT'] = '1'
            headers['Upgrade-Insecure-Requests'] = '1'
            #headers['Cookie'] = '1P_JAR=2018-09-30-15; NID=140=PXj7V4DfXmAD42K3B_NXxqRTXqnZo7aZqKiuaehw21IrZTCJS7Va-n2LhtxOWS472uv47TiBLp974LSI44WOo5iDMMefdtzmgAnWjPfaEGkcaRF-kJqmGvz2XVDtiiOus4Tpat-Iz5no2y-exoc61olpXomBUmimdTGxsqzfD4mNjqSP3X4FMPZvw7xLG0hrDLbG; OGP=-5061451:; OGPC=19007661-2:19007895-1:19008374-2:; OTZ=4592143_20_20__20_; _ga=GA1.1.841862160.1538319124; _gid=GA1.1.55173768.1538319124; _gcl_au=1.1.41426269.1538319124; DV=I7roLUooU1gXYEuIbfArY16gceqxYhY'
            print('similar_images_upload_image posting...')
            response = requests.post(searchUrl, files=multipart, allow_redirects=False, headers=headers)
            print('similar_images_upload_image post returned')

            print('similar_images_upload_image response:', str(response.read()))

            #req1 = urllib.request.Request(searchUrl, headers=headers)
            resp1 = urllib.request.urlopen(response)
            content = str(resp1.read())
            print('similar_images_upload_image:', content)
    ...
    def _get_next_item(self,s):
        if not s:
            return "no_links", {}
        start_line = s.find(b'rg_meta notranslate')
        if start_line == -1:  # If no links are found then give an error!
            end_quote = 0
            link = "no_links"
            return link, end_quote
        else:
            start_line = s.find(b'class="rg_meta notranslate">')
            start_object = s.find(b'{', start_line + 1)
            end_object = s.find(b'</div>', start_object + 1)
            object_raw = s[start_object:end_object] #str(s[start_object:end_object])
            ...
            if cur_version >= version: #python3
                ...
                final_object = json.loads(object_raw) #json.loads(object_decode)
                #except:
                #    final_object = ""
            else:  #python2
                try:
    ...
    # Getting all links with the help of '_images_get_next_image'
    # Kimmo H.: add returnUrlsOnly argument
    def _get_all_items(self,page,main_directory,dir_name,limit,arguments, returnUrlsOnly=False):
        items = []
        abs_path = []
        errorCount = 0
        i = 0
        ...
        for pky in prefix_keywords:
            for sky in suffix_keywords:     # 1.for every suffix keywords
                ...
                while i < len(search_keyword):      # 2.for every main keyword
                    ...
                    if not arguments['returnUrlsOnly']:
                    ...
                    items,errorCount,abs_path = self._get_all_items(raw_html,main_directory,dir_name,limit,arguments, returnUrlsOnly=(arguments['returnUrlsOnly'] if arguments['returnUrlsOnly'] else False))    #get all image items and download images
                    #paths[pky + search_keyword[i] + sky] = abs_path
                    paths.append(abs_path)
                    retValue = retValue + items
