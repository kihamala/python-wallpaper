...
2018-08
..Kimmo Hamalainen humbly edited parts of the code for Python v3.7.
...
class Group(object):
    ...
    def getPhotos(self, tags='', per_page='', page='', user_id=''):
        ...
        photosTotal = '0'
        try:
            pagesTotal = data.rsp.photos.pages
            photosTotal = data.rsp.photos.total
            if hasattr(data.rsp.photos, 'photo'):
                ...
                elif data.rsp.photos.photo:
                    photos.append(_parse_photo(data.rsp.photos.photo))
        except FlickrError as err:
            print(str(err))
        return photos, pagesTotal, photosTotal
...
def photos_search(user_id='', auth=False,  tags='', tag_mode='', text='',\
    ...
    if hasattr(data.rsp.photos, 'photo'):
        if isinstance(data.rsp.photos.photo, list):
            print('photos_search: got', len(data.rsp.photos.photo), 'photos')
            for photo in data.rsp.photos.photo:
                photos.append(_parse_photo(photo))
        else:
...
def _doget(method, auth=False, **params):
    ...
    url = '%s%s/?api_key=%s&method=%s&%s%s'% \
          (HOST, API, API_KEY, method, urllib.parse.urlencode(params),
    ...
    request = reqSession.get(url, stream=True)
    html = request.content
    return _get_data(minidom.parseString(html))
...
class Auth():
    def getFrob(self):
        ...
        signature_hash = hashlib.md5(sig_str.encode('utf-8')).hexdigest()
